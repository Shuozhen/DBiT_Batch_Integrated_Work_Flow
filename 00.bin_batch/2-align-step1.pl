...
foreach my $key(keys %hash)
{
	...
	$y = $hash{$key}[1];
	my $degree = 0;
	if($hash{$key}[2]/$max < 0.5)
	{
		$degree = $hash{$key}[2]*2/$max;
	}
	else
	{
		$degree = $hash{$key}[2]/$max;
	}
	#my $degree = ($hash{$key}[2]/$max + 1)/2;	
	#$svg->rect(x=>($flank_x + $x*20),y=>($flank_y+$y*20),width=>10,height=>10,fill=>"white");
	$svg->rect(x=>($flank_x + ($x-1)*20),y=>($flank_y+($y-1)*20),width=>10,height=>10,'opacity',$degree,fill=>"#000080");
	#$svg->rect(x=>($flank_x + ($x-1)*20),y=>($flank_y+($y-1)*20),width=>10,height=>10,fill=>"navyblue");
}
