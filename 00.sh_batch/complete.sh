...
sh /gpfs/ysm/project/fan/sb2723/00.sh/batch/1-Rezip.sh $SAMPLEID $INDIR

# filter based on barcodes, rearrange read format to be compatible with st-pipeline
# using 2-effective.pl
sh /gpfs/ysm/project/fan/sb2723/00.sh/batch/2-RearrangeRead.sh $SAMPLEID $INDIR

# run st-pipeline, choose human or mouse
#sh /gpfs/ysm/project/fan/sb2723/00.sh/batch/3-stpipeline_hg.sh $SAMPLEID $INDIR
#sh /gpfs/ysm/project/fan/sb2723/00.sh/3-stpipeline_mm.sh $SAMPLEID $INDIR

# run st-pipeline, choose human(1) or mouse(2)
if [$SPECIES == 1]
then
        sh /gpfs/ycga/project/fan/sb2723/00.sh/3-stpipeline_hg.sh $SAMPLEID $INDIR
elif [$SPECIES == 2]
then
        sh /gpfs/ycga/project/fan/sb2723/00.sh/3-stpipeline_mm.sh $SAMPLEID $INDIR
else
